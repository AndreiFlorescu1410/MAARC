...
library(readxl)
#ARIMA(43,1,43)
data1 <- import("/Users/andre/OneDrive/Desktop/Facultate/LICENTA/Cod Algoritm/datenoi/iphonexs.csv")
...
fit2 <- arima(count_ma, order=c(43,1,43))
...
fit3 <- arima(count_ma, order=c(7,2,7))
tsdisplay(residuals(fit3), lag.max = 45)
fcast <- forecast(fit3,h=360)
plot(fcast)
...
write.csv(df,"C:\\Users\\andre\\OneDrive\\Desktop\\Facultate\\LICENTA\\Cod Algoritm\\predictions\\iphonexs.csv", row.names = FALSE)
